...
int main(int argc, char *argv[]){
    ...
    double cutoff_inner = Rh*(1. + horizon_marg); // Cutoff outside or inside BH EH

    // Field of intensity values for output
    // static arrays because we do not want to call malloc functions within the loop
    // stack variables will work faster
    // two global, shared arrays
    ...
    FILE *GRfile = fopen("GRoutput.dat",
                         "w");
...
//                   (steps < max_steps) && photon_u[0] < t_final){
	...
	*/
        ...
        if (steps % 1 == 0)
        fprintf(GRfile, "%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \t%+.15e \n",
                lambda, printphoton_u[0], (logscale ? exp(printphoton_u[1]) : printphoton_u[1]), printphoton_u[2], printphoton_u[3], printphoton_u[4], printphoton_u[5], printphoton_u[6], printphoton_u[7], E, L, Q, norm);
