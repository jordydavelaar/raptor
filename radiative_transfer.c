...
double absorption_coeff_kappa(double nu, double Ne, double Thetae, double B,
  ...
  factor =
      Ne /
      (B * sth); // (Ne * pow(ELECTRON_CHARGE, 2.) * nuc * sth)/SPEED_OF_LIGHT;
...
double absorption_coeff_kappa_FIT(double nu, double Ne, double Thetae, double B,
  ...
  double Rhigh = 4.5;
  double Rlow = 4.5;
  double b2 = pow((B / B_unit) / (Ne / Ne_unit), 2.); //(B/B_unit)/(Ne/Ne_unit);
  kappa = Rhigh * b2 / (1 + b2) + Rlow / (1 + b2);
  w = Thetae; // sqrt(2./9./kappa *Thetae * Thetae);
  nuc = ELECTRON_CHARGE * B / (2. * M_PI * ELECTRON_MASS * SPEED_OF_LIGHT);
  sth = sin(theta);
...
double pitch_angle(double *X_u, double *k_u, double *B_u, double *Uplasma_u) {
  ...
  // Compute and clamp result (result can slightly exceed domain of acos due to
  // numerics)
