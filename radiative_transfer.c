...
// following appendix D Moscibrodzka et al. 2011
double jnu_brem_ee(double nu, double ne, double Thetae) {
    ...
    /*for low temperatures theta<0.02 (50 keV) - Gould 1980,81,
     * nonrelativistic*/
    if (Thetae < 0.1) {
        j = 2. / M_PI * Bl(x) * sqrt(2. * Thetae / M_PI) * exp(-x / 2.) *
            gsl_sf_bessel_Kn(0, x / 2.);
    }

    /*this is for temperaures 0.01-100 (50keV-1MeV) - Stepney and Guilbert
     * 1983*/
    if (Thetae >= 0.1 && Thetae <= 2.) {
        j = Thetae * exp(-x) * Gfun(x, Thetae);
    }
    ...
    if (Thetae > 2.) {
        j = 3. / 4. / M_PI * exp(-x) *
            (28. / 3. + 2. * x + x * x / 2. +
             2. * (8. / 3. + 4. / 3. * x + x) * (log(2. * Thetae) - 0.5772) -
             exp(-x) * Ei(-x) * (8. / 3. - 43. * x + x * x));
    }

    return j / 4. / M_PI * ne * ne * SIGMATH * CL * hpl * alphaf;
}
...
double Bl(double x) { return 0.85 + 1.35 * sqrt(x) + 0.38 * x; }
...
double Gfun(double x, double Thetae) {
    ...
    double Gfun1;
    double energy[] = {50,  75,  100, 150, 200, 300, 400,
                       500, 600, 700, 800, 900, 1000}; // keV

    /* low energy data for different temperatures*/

    double A[] = {1.584, 1.357, 1.197, 1.023, 0.883, 0.700, 0.572,
                  0.484, 0.417, 0.361, 0.322, 0.286, 0.259};

    double B[] = {0.578,  0.437,  0.291,  0.204,  0.0835, -0.0494, -0.139,
                  -0.181, -0.209, -0.204, -0.244, -0.257, -0.258};

    double C[] = {4.565, 3.842, 3.506, 3.036, 2.831, 2.545, 2.352,
                  2.175, 2.028, 1.914, 1.795, 1.705, 1.617};

    double D[] = {2.091, 1.855, 1.672, 1.593, 1.487, 1.364, 1.254,
                  1.179, 1.108, 1.030, 0.982, 0.923, 0.879};

    /* high energy data for different temperatures*/
    double alpha[] = {0.0387, 0.0633, 0.0862, 0.128, 0.159, 0.208, 0.234,
                      0.245,  0.248,  0.247,  0.243, 0.239, 0.235};

    double beta[] = {0.523, 0.540, 0.569, 0.596, 0.658, 0.633, 0.643,
                     0.695, 0.729, 0.756, 0.763, 0.755, 0.735};

    double gamma[] = {5.319, 4.412, 3.897, 3.383, 2.974, 2.738, 2.424,
                      2.025, 1.716, 1.457, 1.271, 1.140, 1.060};

    double delta[] = {0.782, 0.689, 0.633, 0.523, 0.532, 0.326, 0.302,
                      0.394, 0.453, 0.500, 0.515, 0.508, 0.478};
    ...
    if (x > 0.05 && x < 1.1) {
        ...
        if (en < 50) {
            ...
            for (ii = 0; ii < 12; ii++) {
                if (en >= energy[ii] && en <= energy[ii + 1]) {
                    ...
                    x1 = energy[i];
                    x2 = energy[i + 1];
                    // fprintf(stdout,"1:i=%d ii=%d en=%g x1=%g
                    // x2=%g\n",i,ii,en,x1,x2);
                    break;
                }
            }
        ...
        if (en < 50) {
            ...
            for (ii = 0; ii < 12; ii++) {
                if (en >= energy[ii] && en <= energy[ii + 1]) {
                    ...
                    x1 = energy[i];
                    x2 = energy[i + 1];
                    // fprintf(stdout,"2:i=%d ii=%d en=%g x1=%g
                    // x2=%g\n",i,ii,en,x1,x2);
                    break;
                }
            }
        ...
        alphaa =
            (f2A - f1A) / (x2 - x1) * en + (f1A * x2 - x1 * f2A) / (x2 - x1);
        betab =
            (f2B - f1B) / (x2 - x1) * en + (f1B * x2 - x1 * f2B) / (x2 - x1);
        gammag =
            (f2C - f1C) / (x2 - x1) * en + (f1C * x2 - x1 * f2C) / (x2 - x1);
        deltad =
            (f2D - f1D) / (x2 - x1) * en + (f1D * x2 - x1 * f2D) / (x2 - x1);
...
double emission_coeff_kappa(double nu, double Ne, double Thetae, double B,
    ...
    factor = nuc * sth *
             Ne; // (Ne * pow(ELECTRON_CHARGE, 2.) * nuc * sth)/SPEED_OF_LIGHT;
    ...
    if (J_s > 0)
        fprintf(stderr, "T %g X %g J_s %g %g\n", Thetae, X_c, J_s * factor,
                emission_coeff_THSYNCH(B, theta, Thetae, 1 * nuc, Ne));
    return (J_s * factor);
}
...
double absorption_coeff_kappa(double nu, double Ne, double Thetae, double B,
    ...
    factor =
        Ne /
        (B *
         sth); // (Ne * pow(ELECTRON_CHARGE, 2.) * nuc * sth)/SPEED_OF_LIGHT;
...
void read_in_table(char *filename) {
    ...
    // read in header; min and max values of the 5 variables, sizes of these
    // variables
...
double interpolate_scalar_2d(double **var, double nuratio, double Thetae) {
    ...
    if (j0 < 0) {
        ...
        del[1] = (nuratio - nuratio_min * pow(10., (double)j0 / 10.)) /
                 (nuratio_min * pow(10., (double)(j0 + 1) / 10.) -
                  nuratio_min * pow(10., (double)j0 / 10.));
    }
    ...
    if (i0 < 0) {
        ...
        del[0] = (Thetae - theta_e_min * pow(10., (double)i0 / 10.)) /
                 (theta_e_min * pow(10., (double)(i0 + 1) / 10.) -
                  theta_e_min * pow(10., (double)i0 / 10.));
    }
    // printf("%e\n",del[0]);
    ...
    interp = var[i0][j0] * coeff[0] + var[i0][j0 + 1] * coeff[1] +
...
double interpolate_scalar_5d(double *****A, double nu, double Ne, double Thetae,
    ...
    value =
        A[i0][j0][k0][l0][m0] * (1 - del_i) * (1 - del_j) * (1 - del_k) *
            (1 - del_l) * (1 - del_m) +
        A[i1][j0][k0][l0][m0] * (del_i) * (1 - del_j) * (1 - del_k) *
            (1 - del_l) * (1 - del_m) +
        A[i0][j1][k0][l0][m0] * (1 - del_i) * (del_j) * (1 - del_k) *
            (1 - del_l) * (1 - del_m) +
        A[i0][j0][k1][l0][m0] * (1 - del_i) * (1 - del_j) * (del_k) *
            (1 - del_l) * (1 - del_m) +
        A[i0][j0][k0][l1][m0] * (1 - del_i) * (1 - del_j) * (1 - del_k) *
            (del_l) * (1 - del_m) +
        A[i0][j0][k0][l0][m1] * (1 - del_i) * (1 - del_j) * (1 - del_k) *
            (1 - del_l) * (del_m) +
        A[i1][j1][k0][l0][m0] * (del_i) * (del_j) * (1 - del_k) * (1 - del_l) *
            (1 - del_m) +
        A[i1][j0][k1][l0][m0] * (del_i) * (1 - del_j) * (del_k) * (1 - del_l) *
            (1 - del_m) +
        A[i1][j0][k0][l1][m0] * (del_i) * (1 - del_j) * (1 - del_k) * (del_l) *
            (1 - del_m) +
        A[i1][j0][k0][l0][m1] * (del_i) * (1 - del_j) * (1 - del_k) *
            (1 - del_l) * (del_m) +
        A[i0][j1][k1][l0][m0] * (1 - del_i) * (del_j) * (del_k) * (1 - del_l) *
            (1 - del_m) +
        A[i0][j1][k0][l1][m0] * (1 - del_i) * (del_j) * (1 - del_k) * (del_l) *
            (1 - del_m) +
        A[i0][j1][k0][l0][m1] * (1 - del_i) * (del_j) * (1 - del_k) *
            (1 - del_l) * (del_m) +
        A[i0][j0][k1][l1][m0] * (1 - del_i) * (1 - del_j) * (del_k) * (del_l) *
            (1 - del_m) +
        A[i0][j0][k1][l0][m1] * (1 - del_i) * (1 - del_j) * (del_k) *
            (1 - del_l) * (del_m) +
        A[i0][j0][k0][l1][m1] * (1 - del_i) * (1 - del_j) * (1 - del_k) *
            (del_l) * (del_m) +
        A[i1][j1][k1][l0][m0] * (del_i) * (del_j) * (del_k) * (1 - del_l) *
            (1 - del_m) +
        A[i1][j1][k0][l1][m0] * (del_i) * (del_j) * (1 - del_k) * (del_l) *
            (1 - del_m) +
        A[i1][j1][k0][l0][m1] * (del_i) * (del_j) * (1 - del_k) * (1 - del_l) *
            (del_m) +
        A[i1][j0][k1][l1][m0] * (del_i) * (1 - del_j) * (del_k) * (del_l) *
            (1 - del_m) +
        A[i1][j0][k1][l0][m1] * (del_i) * (1 - del_j) * (del_k) * (1 - del_l) *
            (del_m) +
        A[i1][j0][k0][l1][m1] * (del_i) * (1 - del_j) * (1 - del_k) * (del_l) *
            (del_m) +
        A[i0][j1][k1][l1][m0] * (1 - del_i) * (del_j) * (del_k) * (del_l) *
            (1 - del_m) +
        A[i0][j1][k1][l0][m1] * (1 - del_i) * (del_j) * (del_k) * (1 - del_l) *
            (del_m) +
        A[i0][j1][k0][l1][m1] * (1 - del_i) * (del_j) * (1 - del_k) * (del_l) *
            (del_m) +
        A[i0][j0][k1][l1][m1] * (1 - del_i) * (1 - del_j) * (del_k) * (del_l) *
            (del_m) +
        A[i1][j1][k1][l1][m0] * (del_i) * (del_j) * (del_k) * (del_l) *
            (1 - del_m) +
        A[i1][j1][k1][l0][m1] * (del_i) * (del_j) * (del_k) * (1 - del_l) *
            (del_m) +
        A[i1][j1][k0][l1][m1] * (del_i) * (del_j) * (1 - del_k) * (del_l) *
            (del_m) +
        A[i1][j0][k1][l1][m1] * (del_i) * (1 - del_j) * (del_k) * (del_l) *
            (del_m) +
        A[i0][j1][k1][l1][m1] * (1 - del_i) * (del_j) * (del_k) * (del_l) *
            (del_m) +
