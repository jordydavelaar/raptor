...
void metric_dd(const double X_u[4], double g_dd[4][4]) {
  ...
  double Afactor =
      1. / (1. - 2. / r * (1. + qfactor * pow(r / Rfactor, 3. - gammafactor)));
...
void connection_udd(const double X_u[4], double gamma[4][4][4]) {
  ...
  gamma[1][3][3] =
      ...
      rfactor;
  gamma[2][3][3] = -sint * cost / sigma3 *
                   ((r * r + a * a) * A - sigma * delta * a * a * sint * sint);
  gamma[0][0][1] = (r * r + a * a) / (sigma * sigma * delta) *
                   (2. * r * r - sigma) * rfactor;
  gamma[3][0][1] = a / (sigma * sigma * delta) * (2. * r * r - sigma) * rfactor;
...
// Kerr-Schild coordinates
  ...
  gamma[0][1][3] = a * (-r2 + a2 * cth2) * (r1 * (2. + r1) + a2 * cth2) * sth2 /
                   pow(r2 + a2 * cth2, 3.); //
  ...
  gamma[1][1][3] = a *
                   (a4 - 8. * a2 * r1 + 3. * a4 * r1 - 4. * a2 * r2 + 16. * r3 +
                    8. * a2 * r3 + 8. * r4 * r1 +
  ...
  gamma[1][2][1] = gamma[1][1][2];
  gamma[1][2][2] =
      -(r1 * ((a2 + r1 * (r1 - 2.)) * (a4 + 2. * r4 + a2 * r1 * (3. * r1 - 2.) +
                                       a2 * (a2 + fac5) * cos(2. * th)) -
  ...
  gamma[3][1][2] = -2. * a * r1 * (a2 + 2 * r1 * (2. + r1) + a2 * c2th) * cth *
...
#if (metric == MKS2)
  photon_u[2] = Xg2_approx_rand(photon_u[2]); // We only transform theta - r is
                                              // already exponential and R0 = 0
  photon_u[6] = Ug2_approx_rand(photon_u[6],
