...
void hotspot_velocity(double *X_u, double *Uplasma_u) {
    ...
    // Formula for Keplerian velocity in BL metric, notice that has to be
    // transformedinto MBL,Ks or MKS
    Ucon_bl[0] = AA;
    Ucon_bl[1] = 0.;
    Ucon_bl[2] = 0.;
    Ucon_bl[3] = AA * omega;
...
// In case of KS/MKS coords, convert this four-vector into corresponding
// coordinate system
...
// coordinate system
    ...
    for (qq = 0; qq < nblobs; qq++) {
        ...
        xspot[3] = fmod(X_u[0] / P, 1.) * 2. * M_PI +
                   (double)qq / (double)nblobs * 2. *
                       M_PI; // spot current phi at t=X[0]
