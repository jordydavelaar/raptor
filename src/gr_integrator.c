...
void rk45_step(double *y, void (*f)(double *, double *), double *dt) {
    ...
    double err_tot = err[0] + err[1] + err[2];
    double tol = 1e-4;

    if (err_tot > tol) {
        *dt = 0.9 * (*dt) * pow(tol / err_tot, 1. / 5.);
        rk45_step(y, f, dt);
    }
    if (err_tot < tol / 10.) {
        *dt = 0.9 * (*dt) * pow(tol / err_tot, 1. / 5.);
...
// dlambda_adaptive = -STEPSIZE;
    ...
    rk45_step(photon_u, &f_geodesic, &dlambda_adaptive);
