...
void init_harm_data(char *fname) {
    ...
    //         gam);
    Thetae_unit =
        (gam - 1.) * (PROTON_MASS / ELECTRON_MASS) / (1. + TP_OVER_TE);
    ...
    for (k = 0; k < N1 * N2; k++) {
      ...
      */
        fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf", &p[KRHO][i][j],
...
void get_fluid_params(double X[NDIM], double *Ne, double *Thetae, double *B,
    ...
    *B = sqrt(Bcon[0] * Bcov[0] + Bcon[1] * Bcov[1] + Bcon[2] * Bcov[2] +
    ...
    /*
        if (strcmp(TEMP_MODEL, diskstring) == 0){
            ...
            double two_temp_gam = 0.5 * ((1. + 2. / 3. * (trat + 1.) / (trat
       +2.))
       + 4./3.);
    ...
    */
    ...
    //    double dummy_5 = 0.5 * ((1. + 2. / 3. * (dummy_4 + 1.) / (dummy_4
    //    + 2.))
    //    + gam); if (TEMP_MODEL == 2){
    ...
    // Bcov[1] + Ucon[2] * Bcov[2] + Ucon[3] * Bcov[3]); if(fabs(Ucon[0] *
    // Ucov[0]
    // + Ucon[1] * Ucov[1] + Ucon[2] * Ucov[2] + Ucon[3] * Ucov[3] + 1.)
    // > 1.e-9) printf("\nIN RAPTORMODEL u dot u = %+.15e", Ucon[0] * Ucov[0] +
    // Ucon[1] * Ucov[1] + Ucon[2] * Ucov[2] + Ucon[3] * Ucov[3]);
