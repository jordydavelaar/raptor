...
void init_harm_data(char *fname) {
  ...
  */
    fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf", &p[KRHO][i][j], &p[UU][i][j],
           &p[U1][i][j], &p[U2][i][j], &p[U3][i][j], &p[B1][i][j], &p[B2][i][j],
           &p[B3][i][j]);
    //	fprintf(stderr, "%lf\n", p[KRHO][i][j]);
...
void get_fluid_params(double X[NDIM], double *Ne, double *Thetae, double *B,
  ...
  X[2] = Xg2_approx_rand(
      X[2]); // We only transform theta - r is already exponential and R0 = 0
#endif
  ...
  *B = sqrt(Bcon[0] * Bcov[0] + Bcon[1] * Bcov[1] + Bcon[2] * Bcov[2] +
  ...
  double Bsq = (Bcon[0] * Bcov[0] + Bcon[1] * Bcov[1] + Bcon[2] * Bcov[2] +
                Bcon[3] * Bcov[3]);
  ...
  /*
      if (strcmp(TEMP_MODEL, diskstring) == 0){
          ...
          double two_temp_gam = 0.5 * ((1. + 2. / 3. * (trat + 1.) / (trat +2.))
     + 4./3.);
  ...
  */
  ...
  //    double dummy_5 = 0.5 * ((1. + 2. / 3. * (dummy_4 + 1.) / (dummy_4 + 2.))
  //    + gam); if (TEMP_MODEL == 2){
  //        *Thetae = p[UU][i][j] / (*Ne) * Ne_unit * (dummy_5 - 1.) *
  //        (PROTON_MASS / ELECTRON_MASS) / (1. + dummy_4); if(Bsq /
  //        p[KRHO][i][j] > 1.){
  //            *Thetae = 0.;
  //        }
  //  }
  ...
  if (SPHERICAL_ACC) {
    ...
    bl_coord(X, &r, &theta);
    *B = sqrt(8. * M_PI * rho / r) *
         B_unit; // MODIFIED TO ADD B FIELD TO BONDI ACCRETION
  }

  // printf("\nIN RAPTORMODEL u dot b = %+.15e", Ucon[0] * Bcov[0] + Ucon[1] *
  // Bcov[1] + Ucon[2] * Bcov[2] + Ucon[3] * Bcov[3]); if(fabs(Ucon[0] * Ucov[0]
  // + Ucon[1] * Ucov[1] + Ucon[2] * Ucov[2] + Ucon[3] * Ucov[3] + 1.) > 1.e-9)
  // printf("\nIN RAPTORMODEL u dot u = %+.15e", Ucon[0] * Ucov[0] + Ucon[1] *
  // Ucov[1] + Ucon[2] * Ucov[2] + Ucon[3] * Ucov[3]);
